  ...
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'fittrack_runtime') THEN
    RAISE EXCEPTION USING
      ...
      HINT = 'Provision it with a password before migrating. On a fresh volume '
             'docker/postgres/initdb/00-roles.sh does it; on an existing '
             'one run: CREATE ROLE fittrack_runtime LOGIN NOSUPERUSER '
             'NOBYPASSRLS IN ROLE fittrack_app PASSWORD ''<secret>'';';
  END IF;
END $$;
"""
