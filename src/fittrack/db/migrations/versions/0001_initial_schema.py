...
def _run(script: str) -> None:
    ...
    sqlalchemy.text(), which parses ':name' as a bind parameter -- including
    inside SQL comments. A comment mentioning `tenant_id = :t` was enough to
    fail the whole migration with \"a value is required for bind parameter
    't'\". DDL has no binds; it should not be scanned for them.
    """
    bind = op.get_bind()
