...
# A run interrupted after the CA leaves `ca.crt` behind and nothing else.
# Skipping on that one file would make every later run a no-op and the tree
# permanently incomplete, and the failure would then surface as a missing bind
# mount — nowhere near the cause. So check for everything, not for a token.
tree_is_complete() {
  [ -f "${CERTS}/ca/ca.crt" ] && [ -f "${CERTS}/ca/ca.key" ] || return 1
  for service in ${SERVICES}; do
    for artifact in server.crt server.key ca.crt; do
      [ -f "${CERTS}/${service}/${artifact}" ] || return 1
    done
  done
  return 0
}

if [ "${1:-}" != "--force" ] && tree_is_complete; then
  echo "certs/ is complete; pass --force to regenerate."
  exit 0
fi

if [ -d "${CERTS}" ] && [ "${1:-}" != "--force" ]; then
  echo "certs/ is incomplete; regenerating."
fi
