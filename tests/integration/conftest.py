...
def owner_dsn(env: dict[str, str], ca_file: Path) -> str:
    ...
    """
    password = _required(env, name="POSTGRES_PASSWORD", url_variable="MIGRATION_DATABASE_URL")
    return _dsn(env.get("POSTGRES_USER", "fittrack"), password, env.get("POSTGRES_DB", "fittrack"))
