"""Fixtures for tests that need Postgres, Redis or Qdrant.

They live at the suite root rather than under `tests/integration/` because spec
section 23 puts two of their consumers — `test_tenant_isolation.py` and
`test_channel_isolation.py` — beside it, not inside it.

Each one makes a single suite work from two places: inside the `worker`
...
names in their SAN precisely so `verify-full` holds either way.
"""
